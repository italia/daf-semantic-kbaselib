...
//      Cell ??
      ...
      val row = item.map { e => e.toList.map(_._2).mkString("|") }.mkString(" | ")
...
case class Cell(
  name:       String,
  metadata_1: MetadataLevel1,
  metadata_2: MetadataLevel2,
  uri:        String,
  `type`:     String)

case class CellsGroup(
  level: Int,
  cells: Seq[Cell])

case class MetadataLevel1(ontology: URIWithLabel, concept: URIWithLabel, property: URIWithLabel)

case class MetadataLevel2(vocabulary: URIWithLabel, level: Int)
